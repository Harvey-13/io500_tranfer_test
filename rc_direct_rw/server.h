...
public:
    ...
    void listen(char* ip, int port){
        ...
        std::thread watcher([&](){
            void* ptr = this->buffer_;
            for(;;){
                if(this->stop_)break;
                if((uintptr_t)ptr!=*(this->buffer_+cq_len*grain)){
                    for(int i{};i<grain;++i)putchar(*((char*)(ptr+i)));
                    ptr = ptr + grain;
                }
            }
        });
...
private:
    ...
    //     char resp_buf_[grain]{}, msg_buf_[grain]{};
    ...
    void create_connection(rdma_cm_id* cm_id){
        ...
        buf_mr_ = ibv_reg_mr(pd_, buffer_, strlen(buffer_), IBV_ACCESS_LOCAL_WRITE|
                                                            IBV_ACCESS_REMOTE_READ|
                                                            IBV_ACCESS_REMOTE_WRITE);
        CData cdata{};
        cdata.rkey = buf_mr_->rkey;
        cdata.data = buffer_;
    ...
    char buffer_[grain*cq_len+8]{};
    char* pointer_{buffer_};
    ibv_mr* buf_mr_{};
    ibv_pd *pd_{};
